...
export const deleteGastronomyController = async (req, res) => {
    try {
        ...
        console.error('Erro em deleteGastronomyController:', error);
        const isProd = process.env.NODE_ENV === 'production';
        return res.status(500).json({ error: isProd ? 'Erro ao deletar gastronomia' : (error.message || String(error)), stack: isProd ? undefined : error.stack });
    }
};
